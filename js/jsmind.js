...
; (function ($w) {
    ...
    jm.prototype = {
        ...
        get_node_font_size: function (nodeid) {
            var node = this.mind.get_node(nodeid);
            if (!!node) {
        ...
        remove_node_background_image: function (nodeid) {
            if (this.get_editable()) {
                var node = this.mind.get_node(nodeid);
                if (!!node) {
                    delete node.data['background-image'];
                    delete node.data['width'];
                    delete node.data['height'];
                    delete node.data['background-rotation'];
                    this.view.reset_node_custom_style(node);
                    this.view.update_node(node);
                    this.layout.layout();
                    this.view.show(false);
    ...
    jm.view_provider.prototype = {
        ...
        expand_size: function () {
            ...
            this.size.w = client_w-20;	//fix scrollbar issue
            this.size.h = client_h-5;	//fix scrollbar issue
        },
        ...
        _reset_node_custom_style: function (node_element, node_data) {
            ...
            if ('background-image' in node_data) {
                ...
                node_element.style.removeProperty("backgroundImage");
                node_element.style.removeProperty("width");
                if(node_element.getAttribute("style") && node_element.style.height){
                    //node_element.style.removeAttribute("height");
                    node_element.style.height = '20px';
                }
            }
        },
